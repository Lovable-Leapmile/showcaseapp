...
export const SwipeableTabs = ({
  ...
  return <div ref={swipeRef} className={cn("w-full", className)}>
      <EnhancedTabs value={currentValue} onValueChange={handleTabChange}>
        ...
        gridTemplateColumns: `repeat(${tabs.length}, 1fr)`
          ...
          {tabs.map(tab => <EnhancedTabsTrigger key={tab.value} value={tab.value} className="text-sm sm:text-base font-medium data-[state=active]:bg-blue-500 data-[state=active]:text-white transition-all duration-300 rounded-lg m-1 px-0 my-0 py-[5px]">
              {tab.label}
            </EnhancedTabsTrigger>)}
        </EnhancedTabsList>
        {children}
